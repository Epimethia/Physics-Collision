...
Text::Text(std::string newText, std::string newFont, glm::vec2 pos, int size) {
	...
	TextShaderProg = shaderLoader.CreateProgram(const_cast<char*>("Dependencies/shaders/Text.vs"), const_cast<char*>("Dependencies/shaders/Text.fs"));
	text = newText;
	...
	glm::mat4 proj = glm::ortho(0.0f, 600.0f, 0.0f, 600.0f);
...
void Text::Render() {
	...
	glUseProgram(TextShaderProg);
	glUniform3f(glGetUniformLocation(TextShaderProg, "textColor"), color.x, color.y, color.z);
